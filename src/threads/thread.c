...
/* The loading average of the system *///<---------添加变量
int64_t load_avg;
               ...
               thread_func *function, void *aux) 
{
  ...
  if (priority > (thread_current()->priority))//<---------------加了这一句
  {
    thread_yield();
  }
...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered(&ready_list, &t->elem, priority_less, NULL);//<----------------把上一句注释掉的换成这一句
...
thread_yield (void) 
{
  struct thread *cur = thread_current ();//当前正在running的线程
  enum intr_level old_level;
  ...
  if (cur != idle_thread) 
    ...
    list_insert_ordered(&ready_list, &cur->elem, priority_less, NULL);//<--------------把上一句注释掉的换成这一句
...
thread_set_priority (int new_priority) 
{
  // 多级反馈队列不改变优先级 
  if(!thread_mlfqs)
    thread_set_priority_fixed(thread_current(), new_priority, true);//<-------改成这个样子
}

void
thread_set_priority_fixed(struct thread *current_thread, int new_priority, bool donating)//<--------加了这个函数
{
  // 多级反馈队列不改变优先级 
  if(!thread_mlfqs)
  {
    ...
    {  
        ...
        else
        {
			...
			if(current_thread == thread_current())
            {
                current_thread->old_priority  = new_priority;
            }
        }
...
/* Sets the current thread's nice value to NICE. *///<---------------修改这里
void
